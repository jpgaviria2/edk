...
def draw_face(pdf, card, x, y):
    ...
    text.textLine('')
    for fragment in [card['lesson']]:
        text.textLine(fragment)
    pdf.drawText(text)

    pdf.setFillColor(LIGHT)
    pdf.setStrokeColor(black)
    pdf.roundRect(x + 14, y + 16, CARD_W - 28, 30, 8, stroke=1, fill=1)
    pdf.setFillColor(black)
    pdf.setFont('Helvetica-Bold', 9)
    pdf.drawCentredString(x + CARD_W / 2, y + 27, card['label'].upper())
    pdf.setFont('Helvetica', 7)
    pdf.drawCentredString(x + CARD_W / 2, y + 18, 'Front')
...
def draw_back(pdf, card, x, y):
    ...
    pdf.setFillColor(black)
    pdf.setFont('Helvetica-Bold', 17)
    pdf.drawCentredString(x + CARD_W / 2, y + CARD_H - 54, card['label'])
    pdf.setFont('Helvetica-Bold', 12)
    pdf.drawCentredString(x + CARD_W / 2, y + CARD_H - 72, 'History of Money')
    pdf.setFont('Helvetica', 10)
    pdf.drawCentredString(x + CARD_W / 2, y + CARD_H - 88, 'Card Trading Kit')
    ...
    pdf.drawCentredString(x + CARD_W / 2, y + 34, 'Block 02 · printable B&W deck back')
    pdf.drawCentredString(x + CARD_W / 2, y + 22, 'Flip on long edge')
