# Bitcoin Transaction Demo

Physical Block 7 kit for showing Bitcoin transactions live at 8 tables of 10 people.

Files:

- `BITCOIN-TRANSACTION-DEMO-KIT.md` — facilitator rules, review, run-of-show, packing guidance
- `PACKING-LIST-8-TABLES.csv` — quantity checklist for procurement/assembly
- `BITCOIN-TRANSACTION-DEMO-KIT.pdf` — print-friendly facilitator PDF
- `generate_pdf.py` — local PDF generator

Recommended use: run this as the classroom/live-event extension for Block 7: Transactions — Send & Receive.
